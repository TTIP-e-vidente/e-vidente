...
import { GameRow, InsertGameInput, InsertGameResult } from '../types/player.ts';
...
export async function insertGame(
  ...
  if (input.nodeId) {
    ...
    const historyResult = await client.queryObject<{ id: string }>(
      `
  ...
  const result = await client.queryObject<GameRow>(
    `
      INSERT INTO games (
        history_id,
        user_id,
