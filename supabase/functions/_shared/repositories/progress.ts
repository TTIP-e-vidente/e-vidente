...
import * as historyGameRepository from './history-game.ts';
import {
  CompletedNodeRow,
...
export async function upsertProgress(
  ...
  const result = await client.queryObject<ProgresoRestriccionRow>(
    `
...
export async function incrementProgressCompletedNodes(
  ...
  const result = await client.queryObject<{ map_completed: boolean }>(
    `
...
export async function getCompletedNodeByProgressAndNode(
  ...
  const result = await client.queryObject<CompletedNodeRow>(
    `
      SELECT
        id,
        user_id,
