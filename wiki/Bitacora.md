...
Se conectó el juego con una primera capa de backend para que el progreso pueda guardarse también en una cuenta. El punto importante de esta etapa es que el backend suma una persistencia remota, pero no reemplaza el guardado local ni bloquea la experiencia: si el servidor no está disponible, el jugador puede seguir jugando.
...
- `juego/project/backend/`, `juego/project/auth/Login.gd`, `juego/project/player/Profile.gd`
