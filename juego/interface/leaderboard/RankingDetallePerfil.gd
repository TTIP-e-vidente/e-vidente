...
func cargar(scope: String = "") -> void:
	...
	_ocultar_invitado()
	await _refrescar_scope(scope_final)
...
func _mostrar_invitado() -> void:
	...
	if is_instance_valid(_mini_list):
		_mini_list.limpiar()
	if is_instance_valid(_boton_tabla):
		_boton_tabla.text = "Iniciar sesión para ver ranking"
...
func _refrescar_scope(scope: String) -> void:
	if is_instance_valid(_card):
		await _card.cargar_y_mostrar(scope)
