...
func update_local_profile(username: String, age: int, email: String, avatar_source_path: String) -> Dictionary:
	...
	if clean_avatar_path.is_empty():
		_manager._remove_managed_avatar(previous_avatar_path)
		profile["avatar_path"] = ""
	else:
		var persisted_avatar_path: String = _manager._persist_avatar(_manager.current_user_key, clean_avatar_path)
	...
	_manager._append_history("Perfil local actualizado", {"type": "profile_updated"})
	_manager.save_current_user_progress(false)
	if not _manager._write_save_data(false, "profile_updated"):
...
func load_avatar_texture(path: String) -> Texture2D:
	return _manager._profile_helper.load_avatar_texture(path)
...
func validate_profile(username: String, age: int, email: String, avatar_source_path: String) -> Dictionary:
	return _manager._profile_helper.validate_profile(username, age, email, avatar_source_path)
...
func ensure_local_profile() -> bool:
	...
	profile = _manager._normalize_profile_data(profile)
	...
	if not save_meta is Dictionary:
		...
		_manager.save_data["save_meta"] = _manager._normalize_save_meta(save_meta)
	...
	if not resume_state is Dictionary:
		...
		_manager.save_data["resume_state"] = _manager._normalize_resume_state(resume_state)
