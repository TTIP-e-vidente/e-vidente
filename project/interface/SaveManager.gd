...
func mark_activity_completed(request_key: String, activity_id: String) -> void:
	...
	if clean_id.is_empty():
		push_error("[SaveManager] mark_activity_completed: activity_id vacío")
		return
	var clean_key: String = request_key.strip_edges()
	if clean_key.is_empty():
		# Juegos fijos y legacy no tienen request_key; se agrupan bajo __global__.
	...
	print("[SaveManager] mark_completed activity_id=%s request=%s" % [clean_id, clean_key])
	guardar_progreso_en_disco()
...
func mark_map_reward_seen(map_id: String) -> void:
	...
	_marcar_guardado_sucio()
	guardar_progreso_en_disco()
	print("[MapCompletion] mark_map_reward_seen map_id=", clean_id)


func was_map_reward_seen(map_id: String) -> bool:
...
func get_all_used_activity_ids() -> Array[String]:
	...
	for key in (stored as Dictionary).keys():
		var raw_ids: Variant = (stored as Dictionary).get(key, [])
		if not raw_ids is Array:
			continue
...
func get_all_completed_activity_ids() -> Array[String]:
	return get_all_used_activity_ids()
...
func mark_activity_played(request_key: String, activity_id: String) -> void:
	var clean_id: String = activity_id.strip_edges()
	if clean_id.is_empty():
		push_error("[SaveManager] mark_activity_played: activity_id vacío")
		return
	print("[SaveManager] mark_played activity_id=%s" % clean_id)
	mark_activity_completed(request_key, activity_id)
