...
func normalize_partial_level_state(raw_state: Variant) -> Dictionary:
	...
	var mechanic_type: String = str(
		raw_state.get(_manager.PARTIAL_LEVEL_MECHANIC_TYPE_KEY, "")
	).strip_edges()
	...
	if mechanic_type == LevelMechanicTypes.PLATE_SORT:
		...
		var placed_item_ids: Variant = normalized_mechanic_state.get(
			_manager.PARTIAL_LEVEL_PLACED_ITEM_IDS_KEY,
			[]
		)
		normalized_state[_manager.PARTIAL_LEVEL_PLACED_ITEM_IDS_KEY] = placed_item_ids
...
func prune_partial_level_states(partial_level_states: Dictionary) -> void:
	for track_key in _manager.TRACK_KEYS:
		...
		for raw_level_key in track_states.keys():
			...
			if _manager.is_level_completed(track_key, level_number):
				track_states.erase(raw_level_key)
...
func _normalize_mechanic_state(
	...
	if clean_mechanic_type.is_empty() or clean_mechanic_type == LevelMechanicTypes.PLATE_SORT:
		...
		if not plate_sort_source is Dictionary or (plate_sort_source as Dictionary).is_empty():
			plate_sort_source = {
				_manager.PARTIAL_LEVEL_ITEMS_KEY: raw_state.get(
					_manager.PARTIAL_LEVEL_ITEMS_KEY,
					[]
				),
				_manager.PARTIAL_LEVEL_PLACED_ITEM_IDS_KEY: raw_state.get(
					_manager.PARTIAL_LEVEL_PLACED_ITEM_IDS_KEY,
					[]
				)
			}
...
func _normalize_plate_sort_mechanic_state(raw_mechanic_state: Variant) -> Dictionary:
	...
	var raw_placed_item_ids: Variant = raw_mechanic_state.get(
		_manager.PARTIAL_LEVEL_PLACED_ITEM_IDS_KEY,
		[]
	)
