...
func fallar_por_timeout() -> void:
	if prueba_finalizada:
		return
	finalizar_con_error("La prueba del indicador de nivel superó el tiempo máximo.")
...
func ejecutar_prueba() -> void:
	...
	indicador.actualizar("Prepara el desayuno", 1, 3)
	await process_frame
	...
	if not fallo:
		_verificar(texto_contexto.visible, "El título del nivel debería quedar visible.")
		_verificar(
			texto_contexto.text == "Prepara el desayuno",
			"El indicador debería mostrar el título del nivel."
		)
		_verificar(texto_progreso.text == "Juego 1/3", "El indicador debería mostrar Juego 1/3.")
...
func _verificar(condicion: bool, mensaje: String) -> void:
	if condicion:
		return
	fallo = true
	printerr("INDICADOR NIVEL TEST FAILED: %s" % mensaje)
