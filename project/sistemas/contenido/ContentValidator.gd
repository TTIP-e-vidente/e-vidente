...
static func _validate_assets_catalog(data: Dictionary) -> Array[String]:
	...
	for item_id in (items as Dictionary).keys():
		...
		if asset_path.is_empty() or not FileAccess.file_exists(asset_path):
			errors.append("Asset no encontrado en assets_catalog.json: \"%s\"" % item_id)
	return errors
...
static func _validate_drag_drop_game(data: Dictionary) -> Array[String]:
	...
	if str(data.get("objetivo", "")).strip_edges().is_empty():
		errors.append("Falta campo obligatorio: objetivo")
	...
	if correct_items.is_empty():
		errors.append("En receta_arrastre: falta correctos")
	if wrong_items.is_empty():
		errors.append("En receta_arrastre: falta incorrectos")
	var correct_count: int = int(data.get("cantidad_correctos", 0))
	var wrong_count: int = int(data.get("cantidad_incorrectos", 0))
	if correct_count > correct_items.size():
		errors.append(
			"En receta_arrastre: cantidad_correctos es mayor que la cantidad de ids disponibles"
		)
	if wrong_count > wrong_items.size():
		errors.append(
			"En receta_arrastre: cantidad_incorrectos es mayor que la cantidad de ids disponibles"
		)
...
static func _validate_linking_game(data: Dictionary) -> Array[String]:
	...
	if str(data.get("categoria", "")).strip_edges().is_empty():
		errors.append("Falta campo obligatorio: categoria")
	if str(data.get("instruccion", "")).strip_edges().is_empty():
		errors.append("Falta campo obligatorio: instruccion")
	...
	if conceptos_izquierda is Array and conceptos_derecha is Array:
		if (conceptos_izquierda as Array).size() != (conceptos_derecha as Array).size():
			errors.append("Vinculacion: ambos lados deben tener la misma cantidad de conceptos")
	return errors
...
static func _validate_questions_game(data: Dictionary) -> Array[String]:
	...
	if not questions is Array or (questions as Array).is_empty():
		errors.append("Falta campo obligatorio: preguntas")
		return errors
	for raw_question in questions as Array:
		...
		if str(question.get("id", "")).strip_edges().is_empty():
			errors.append("Pregunta sin id")
		if str(question.get("enunciado", "")).strip_edges().is_empty():
			errors.append("Pregunta sin enunciado")
		var options: Variant = question.get("opciones", [])
		if not options is Array or (options as Array).size() < 2:
			errors.append("Pregunta con opciones insuficientes: %s" % question.get("id", ""))
			continue
		var correct_option_count := 0
		for raw_option in options as Array:
			if not raw_option is Dictionary:
				continue
			if bool((raw_option as Dictionary).get("correcta", false)):
